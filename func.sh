...
function install_keystone()
{
	...
	echo "CREATE DATABASE keystone;GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'$host' IDENTIFIED BY '$pass';FLUSH PRIVILEGES;"|mysql -uroot -p$pass
...
function install_glance()
{
        ...
        host=$1
        pass=$2
	echo "CREATE DATABASE glance;GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'$host' IDENTIFIED BY '$pass';FLUSH PRIVILEGES;"|mysql -uroot -p$pass
...
function install_controller()
{
	...
	subnet=`echo $host|awk -F. '{print $1"."$2"."$3".%"}'`
        pass=$2
	conf=/etc/nova/nova.conf
	echo "CREATE DATABASE nova;GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'localhost' IDENTIFIED BY '$pass';GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'$subnet' IDENTIFIED BY '$pass';FLUSH PRIVILEGES;"|mysql -uroot -p$pass
	...
	nova-manage network create private --fixed_range_v4=${COMPUTE_PRIVATE_NET} --bridge_interface=br100 --num_networks=1 --multi_host=T --network_size=256
}
...
function install_compute()
{
	...
	apt-get install -y nova-compute-kvm nova-network nova-novncproxy novnc nova-api
	cp ./conf/nova.conf /etc/nova/
